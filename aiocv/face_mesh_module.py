...
class FaceMesh():
    def __init__(self,staticMode=False,maxFaces=2,thickness=1,circle_radius=2,detectionConfidence=0.5,trackConfidence=0.5) -> None:
        ...
        self.faceMesh=self.mpFaceMesh.FaceMesh(self.staticMode,self.maxFaces,self.detectionConfidence,self.trackConfidence)
    ...
    def findFaceMesh(self,img,draw=True) -> "Image":
        ...
        if self.results.multi_face_landmarks:
            for face in self.results.multi_face_landmarks:
                if draw:
                    self.mpDraw.draw_landmarks(img,face,self.mpFaceMesh.FACE_CONNECTIONS,self.drawSpec,self.drawSpec)
